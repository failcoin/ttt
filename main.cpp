...
uint256 hashGenesisBlock("0x3193805be8d39841fcf88d9769d89fafeb11a91d2a7c720f633b99c04efbc94a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AntiKeiserCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AntiKeiserCoin Signed Message:\n";
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // AntiKeiserCoin difficulty adjustment protocol switch
    ...
    // AntiKeiserCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlock *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
 ...
 unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlock *pblock)
 {
         ...
         int64                       PastSecondsMax                               = TimeDaySeconds * 1.5;
 ...
 {
         int DiffMode = 1;
		 if (pindexLast->nHeight+1 >= 12000) { DiffMode = 2; }
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal antikeisercoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AntiKeiserCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "testeleseses ez gecci!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd4b76b90ff97bbcefae79787a8db422a7b0780d0629f9be65b49c7b339b685a4"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xc0, 0xc0 }; // AntiKeiserCoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Disconnect if not /AKC:
        if (pfrom->strSubVer.empty() || pfrom->strSubVer.find("/AKC:") == string::npos)
        {
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // AntiKeiserCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AntiKeiserCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("AntiKeiserCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadAntiKeiserCoinMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static AntiKeiserCoinMiner(CWallet *pwallet)
{
    printf("AntiKeiserCoinMiner started\n");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running AntiKeiserCoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadAntiKeiserCoinMiner(void* parg)
{
    ...
    try
    {
        ...
        AntiKeiserCoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadAntiKeiserCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadAntiKeiserCoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadAntiKeiserCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d AntiKeiserCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadAntiKeiserCoinMiner, pwallet))
